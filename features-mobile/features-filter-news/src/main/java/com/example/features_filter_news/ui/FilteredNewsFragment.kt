...
import com.example.features_common.adapter.filteredNews.FilteredNewsAdapter
import com.example.features_common.adapter.filteredNews.FilteredNewsStateAdapter
...
class FilteredNewsFragment : Fragment() {
    ...
    private lateinit var filteredNewsAdapter: FilteredNewsAdapter
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (preferencesManager.getString(STATE_LANGUAGE) != null && preferencesManager.getString(
                STATE_LANGUAGE
            ) != ""
        ) {
            countFilter++
        ...
        if (preferencesManager.getString(EXTRA_FROM_DATE) != null && preferencesManager.getString(
                EXTRA_FROM_DATE
            ) != ""
        ) {
            countFilter++
        ...
        if (preferencesManager.getString(STATE_SEGMENTS) != null && preferencesManager.getString(
                STATE_SEGMENTS
            ) != ""
        ) {
            countFilter++
        ...
        lifecycleScope.launch {
            viewModel.filteredNews(
                language = preferencesManager.getString(STATE_LANGUAGE) ?: "",
                ...
                filteredNewsAdapter.submitData(it)
            }
...
//                    (requireActivity() as ClickListenerFromFilteredNews).clickListenerToShowError(
//                        state.error
//                    )
//                }
//
...
//        }
    ...
    private fun setRecyclerView() {
        filteredNewsAdapter = FilteredNewsAdapter(requireContext())
        with(binding.filteredNewsRv) {
            adapter = filteredNewsAdapter.withLoadStateFooter(
                FilteredNewsStateAdapter()
            )
            layoutManager = LinearLayoutManager(requireContext())
        }

        filteredNewsAdapter.onClickListenerFilteredNews = {
            (requireActivity() as ClickListenerFromFilteredNews).clickListenerToNewsPost(it)
        }
